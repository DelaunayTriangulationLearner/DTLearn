...
class RandomCrystal:
    ...
    def fit(self, X_train, Y_train):
        ...
        for k in range(self.n_estimator):
            ...
            #bootstrapped_dims = np.random.choice(range(np.size(X_train, axis=1)), int(np.size(X_train, axis=1)*self.p_bootstrap), replace=False)
...
if __name__ == '__main__':
    ...
    db = RandomCrystal(n_estimator=5, p_bootstrap=0.9)
